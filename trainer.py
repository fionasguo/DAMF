...
class DomainAdaptTrainer:
    ...
    def update_lambda_domain(self, epoch, i_dataloader):
        ...
        if epoch >= self.args['num_no_adv']:
            tot_steps_for_calc = (
                self.args['n_epoch'] - self.args['num_no_adv']) * self.len_dataloader
            curr_steps = float(
                i_dataloader + (epoch - self.args['num_no_adv']) * self.len_dataloader)
    ...
    def compute_domain_adapt_loss(self, data, epoch, mf_loss=True):
        ...
        outputs = self.model(
            data['input_ids'],
            data['attention_mask'],
            data['domain_labels'], self.args['lambda_domain'],
            adv=is_adv
        )
        ...
        loss_domain = 0.0
        if is_adv:
            loss_domain = self.loss_fn_domain(
                outputs['domain_output'], data['domain_labels'].squeeze())
            loss += loss_domain
        # mf loss
        loss_mf = 0.0
        if mf_loss:
            loss_mf = self.loss_fn_mf(
                outputs['class_output'], data['mf_labels'])
            loss += loss_mf
        # rec loss
        loss_rec = 0.0
    ...
    def train_basic(self):
        ...
        self.train_dataloader = DataLoader(
            dataset=self.datasets['train'], batch_size=self.args['batch_size'], shuffle=True, drop_last=True, num_workers=4, worker_init_fn=worker_init_fn)
        ...
        for epoch in range(self.args['n_epoch']):
            ...
            if accu > best_accu:
                ...
                best_epoch = epoch
                torch.save(
                    self.model, self.args['output_path'] + '/best_model.pth')
    ...
    def train_semisupervised(self):
        ...
        self.dataloaders['s_train'] = DataLoader(
            dataset=self.datasets['s_train'], batch_size=s_train_batch_size, shuffle=True, drop_last=True, num_workers=4, worker_init_fn=worker_init_fn)
        self.dataloaders['t_train'] = DataLoader(
            dataset=self.datasets['t_train'], batch_size=t_train_batch_size, shuffle=True, drop_last=True, num_workers=4, worker_init_fn=worker_init_fn)
        self.len_dataloader = min(
            len(self.dataloaders['s_train']), len(self.dataloaders['t_train']))
        ...
        for epoch in range(self.args['n_epoch']):
            ...
            for i in range(self.len_dataloader):
                ...
                self.args['lambda_domain'] = self.update_lambda_domain(
                    epoch, i)
                logging.debug('\r epoch: %d, [iter: %d / all %d], lambda_domain = %f' % (
                    epoch, i + 1, self.len_dataloader, self.args['lambda_domain']))
                ...
                logging.debug('\r epoch: %d, [iter: %d / all %d], total_loss: %.3f, total_s_loss: %.3f, total_t_loss: %.3f\ns_loss_mf: %.3f, s_loss_domain: %.3f, s_loss_rec: %.3f, s_loss_trans: %.3f\nt_loss_domain: %.3f, t_loss_rec: %.3f, t_loss_trans: %.3f'
                              % (epoch, i + 1, self.len_dataloader, self.print_loss(loss), self.print_loss(s_loss),
                                 (s_train_batch_size / t_train_batch_size) *
                                  self.print_loss(t_loss),

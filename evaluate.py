    ...
    while i < len_dataloader:
        ...
        with torch.no_grad():
            if not domain_adapt:
                ...
                outputs = model(data_target['input_ids'],data_target['attention_mask'], data_target['domain_labels'], lambda_domain=0, adv=is_adv)
        ...
        if domain_adapt and is_adv:
            ...
            domain_labels.extend(data_target['domain_labels'].to('cpu').tolist())
...
def evaluate(dataset,batch_size,model=None,model_path=None,domain_adapt=True,is_adv=True,test=False):
    ...
    assert (model != None or model_path != None), 'Provide a model object or a model path.'
    if model == None:
    ...
    mf_preds, mf_labels, domain_preds, domain_labels = predict(model, dataset, batch_size, device, domain_adapt, is_adv)

    # print
    mf_report = metrics.classification_report(mf_labels, mf_preds, zero_division=0)
    ...
    mf_report = metrics.classification_report(mf_labels, mf_preds, zero_division=0,output_dict=True)
    macro_f1 = mf_report['weighted avg']['f1-score']

    if domain_adapt and is_adv:
        domain_report = metrics.classification_report(domain_labels, domain_preds, zero_division=0)
...
def compute_feat(model,dataset,batch_size,device):
    ...
    while i < len_dataloader:
        ...
        with torch.no_grad():
            try:
                last_hidden_state,feat = model.module.gen_feature_embeddings(data['input_ids'],data['attention_mask'])
            except:
                last_hidden_state,feat = model.gen_feature_embeddings(data['input_ids'],data['attention_mask'])
...
def feature_embedding_analysis(source_dataset,target_dataset,batch_size,fig_save_path,model=None,model_path=None):
    ...
    s_feats,s_domain_labels = compute_feat(model,source_dataset,batch_size,device)
    t_feats,t_domain_labels = compute_feat(model,target_dataset,batch_size,device)
